                ...
                if (findDelegate) {
                        ...
                        if (req.body.sendMail == 'true') {
                                ...
                                if (req.body.receiptNo == (null || undefined)) {
                                        ...
                                        let mailSend = await commonFunction.receiptGiven(req.body.firstName, req.body.middleName, req.body.lastName, req.body.designation, req.body.address1, req.body.email, req.body.delegatePassword)
                                        if (mailSend) {
                                                req.body.password = await bcrypt.hash(delegatePassword, 10);
                                                req.body.userType = "DELEGATE";
                                                const newCategory = await User.create(req.body);
...
exports.deleteDgDesk = async (req, res) => {
        try {
                ...
                const user = await dgDesk.findOne({ _id: dgDeskId, type: "DG" });
                if (user) {
...
exports.deleteNotification = async (req, res) => {
        try {
                ...
                const user = await notification.findById({ _id: notificationId });
                if (user) {
                        const user1 = await notification.findByIdAndDelete({ _id: user._id });;
                        if (user1) {
                                return res.status(200).json({ message: "notification delete successfully.", status: 200, data: {}, });
                        }
                } else {
                        return res.status(404).json({ message: "notification not Found", status: 404, data: {}, });
                }
        } catch (error) {
                console.error(error);
                return res.status(500).json({ error: "Failed to retrieve DgDesk" });
